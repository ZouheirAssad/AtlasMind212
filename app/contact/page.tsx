...
import { Clock3, Mail, Sparkles, CheckCircle2 } from "lucide-react";
...
const nextSteps = [
  { title: "We review", description: "Your message lands in our inbox. We read the context and goals before anything else." },
  { title: "We reply", description: "You get a direct, no-template response from a human — usually within 2 business days." },
  { title: "We scope", description: "If the fit is right, we send a short recommendation with next steps and a timeline." },
];
      ...
      <section className="relative overflow-hidden bg-neutral-surface py-16 sm:py-24">
        ...
        <Container className="relative grid gap-12 lg:grid-cols-[0.58fr_1.42fr]">
          <Reveal className="flex flex-col gap-10 lg:sticky lg:top-28 lg:self-start">
            ...
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.18em] text-primary">Good fit</p>
              <h2 className="mt-3 text-4xl tracking-[-0.035em]">AI workflows, n8n builds, content systems, and practical tool strategy.</h2>
              ...
              <ol className="mt-5 flex flex-col gap-5">
                {nextSteps.map((step, index) => (
                  <li key={step.title} className="flex gap-4">
                    <span className="flex size-7 shrink-0 items-center justify-center rounded-full border border-primary/40 bg-secondary font-mono text-xs font-semibold text-primary">{index + 1}</span>
                    <div>
