  ...
  return (
    <>
      ...
      <article>
        ...
        <section className="bg-white py-20 sm:py-28">
          <Container className="grid gap-14 lg:grid-cols-2">
            ...
            <Reveal delay={0.08}>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">{service.deliverables.map((item) => <div key={item} className="border-l-4 border-primary-display bg-[var(--surface-raised)] p-5 font-semibold">{item}</div>)}</div>
            </Reveal>
          </Container>
        </section>
        ...
        <section className="bg-[var(--surface-raised)] py-14">
          <Container className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Next step</p><h2 className="mt-2 text-3xl">Scope a {service.title.toLowerCase()} project.</h2></div><Button asChild size="lg"><Link href={`/contact?project=${service.slug}`}>Contact AtlasMind <ArrowRight /></Link></Button></Container>
        </section>
