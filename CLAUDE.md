...
Guide file uploads go to the `guide-assets` Storage bucket; size/MIME limits are constants in `lib/guides.ts` (`GUIDE_THUMBNAIL_*`, `GUIDE_PDF_*`). Orphaned uploads are rolled back if the DB insert fails. `next.config.ts` raises the server-action body limit to `35mb` for these uploads.
...
- **Blog/guide content** is DB-managed via the admin CMS (`public.guides` + Supabase Storage `guide-assets`), surfaced through `lib/guides.ts`.
